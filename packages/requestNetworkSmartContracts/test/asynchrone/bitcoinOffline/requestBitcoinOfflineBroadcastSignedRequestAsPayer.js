...
contract('RequestBitcoinOffline broadcastSignedRequestAsPayerAction',  function(accounts) {
	...
	it("can broadcast request with tips", async function () {
		...
		var hash = hashRequest(requestBitcoinOffline.address, payees, expectedAmounts, payeesPayment, 0, data, timeExpiration);
		var signature = await signHashRequest(hash,payee);

		
		var r = await requestBitcoinOffline.broadcastSignedRequestAsPayerAction(
		...
		assert.equal(newReq[1],requestBitcoinOffline.address,"can broadcast request wrong data : currencyContract");
	...
	it("cannot broadcast request if payee==payer", async function () {
		...
		await utils.expectThrow(requestBitcoinOffline.broadcastSignedRequestAsPayerAction(
									createBytesRequest(payees, expectedAmounts, 0, data),
									utils.createBytesForPaymentBitcoinAddress(payeesPayment),
									utils.createBytesForPaymentBitcoinAddress(payeesRefund),
									additionals,
	...
	it("cannot broadcast request when currencyContract not trusted", async function () {
		...
		var hash = hashRequest(requestBitcoinOffline2.address, payees, expectedAmounts, payeesPayment, 0, data, timeExpiration);
		var signature = await signHashRequest(hash,payee);
		
		await utils.expectThrow(requestBitcoinOffline2.broadcastSignedRequestAsPayerAction(
									createBytesRequest(payees, expectedAmounts, 0, data),
									utils.createBytesForPaymentBitcoinAddress(payeesPayment),
									utils.createBytesForPaymentBitcoinAddress(payeesRefund),
									additionals,
	...
	it("can broadcast request with fees", async function () {
		...
		await requestBitcoinOffline.setRateFees(1, 1000, {from:admin}); // 0.1%
		await requestBitcoinOffline.setMaxCollectable('10000000000000000', {from:admin}); // 0.01 ether
		...
		var fees = await requestBitcoinOffline.collectEstimation(arbitraryAmount + arbitraryAmount2);
