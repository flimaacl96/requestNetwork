...
contract('RequestBitcoinOffline Accept',  function(accounts) {
	...
	it("accept if Core Paused OK", async function () {
		...
		var r = await requestBitcoinOffline.acceptAction(utils.getRequestId(requestCore.address, 1), {from:payer});
	...
	it("accept request Bitcoin Offline pause impossible", async function () {
		await requestBitcoinOffline.pause({from:admin});
		await utils.expectThrow(requestBitcoinOffline.acceptAction(utils.getRequestId(requestCore.address, 1), {from:payer}));
	});

	it("accept request not exist impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.acceptAction(666, {from:payer}));
	});

	it("accept request from a random guy impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.acceptAction(utils.getRequestId(requestCore.address, 1), {from:burnerContract}));
	});
	it("accept request from payee impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.acceptAction(utils.getRequestId(requestCore.address, 1), {from:payee}));
	});
