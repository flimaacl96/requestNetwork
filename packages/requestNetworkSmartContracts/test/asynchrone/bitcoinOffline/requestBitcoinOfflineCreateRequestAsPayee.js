...
contract('RequestBitcoinOffline createRequestAsPayee',  function(accounts) {
	...
	it("new request with address missing", async function () {
		await utils.expectThrow(requestBitcoinOffline.createRequestAsPayeeAction(
														[payee,payee2,payee3],
	...
	it("new request when currencyContract not trusted Impossible", async function () {
		...
		await utils.expectThrow(requestBitcoinOffline2.createRequestAsPayeeAction(
												[payee,payee2,payee3],
												utils.createBytesForPaymentBitcoinAddress([payeePayment,payee2Payment,payee3Payment]), 
												[arbitraryAmount,arbitraryAmount2,arbitraryAmount3], 
												payer, 
	...
	it("new request with fees", async function () {
		...
		await requestBitcoinOffline.setMaxCollectable('2000000000000000', {from:admin});
	...
	it("impossible change fees if not admin", async function () {
		await utils.expectThrow(requestBitcoinOffline.setRateFees(1,10000,{from:payee})); 
	});
	it("impossible change maxCollectable if not admin", async function () {
		await utils.expectThrow(requestBitcoinOffline.setMaxCollectable('2000000000000000',{from:payee})); 
	});
