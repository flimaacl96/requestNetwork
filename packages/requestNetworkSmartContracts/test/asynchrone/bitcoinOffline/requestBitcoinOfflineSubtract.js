...
contract('RequestBitcoinOffline SubtractAction',  function(accounts) {
	...
	it("subtract if Core Paused OK", async function () {
		...
		var r = await requestBitcoinOffline.subtractAction(utils.getRequestId(requestCore.address, 1),[arbitraryAmount10percent], {from:payee});
	...
	it("subtract request not exist impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.subtractAction(666, [arbitraryAmount10percent], {from:payee}));
	});
	...
	it("subtract by payee request canceled impossible", async function () {
		...
		await utils.expectThrow(requestBitcoinOffline.subtractAction(utils.getRequestId(requestCore.address, 1), [arbitraryAmount10percent], {from:payee}));
	});

	it("subtract request from a random guy Impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.subtractAction(utils.getRequestId(requestCore.address, 1), [arbitraryAmount10percent], {from:burnerContract}));
	});

	it("subtract request from payer Impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.subtractAction(utils.getRequestId(requestCore.address, 1), [arbitraryAmount10percent], {from:payer}));
	});
	...
	it("subtract request with amount > expectedAmount Impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.subtractAction(utils.getRequestId(requestCore.address, 1),[arbitraryAmount+1], {from:payee}));
	});

	it("subtract request with amount > expectedAmount on subPayee Impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.subtractAction(utils.getRequestId(requestCore.address, 1),[0, arbitraryAmount2+1], {from:payee}));
	});

	it("subtract request with amount <= expectedAmount OK", async function () {
		var r = await requestBitcoinOffline.subtractAction(utils.getRequestId(requestCore.address, 1),[arbitraryAmount], {from:payee});
	...
	it("subtract subPayees", async function () {
		var r = await requestBitcoinOffline.subtractAction(utils.getRequestId(requestCore.address, 1),[0,arbitraryAmount10percent,arbitraryAmount20percent], {from:payee});
	...
	it("cannot subtract with more amounts than expected", async function () {
		await utils.expectThrow(requestBitcoinOffline.subtractAction(utils.getRequestId(requestCore.address, 1),[arbitraryAmount10percent,arbitraryAmount10percent,arbitraryAmount10percent,arbitraryAmount10percent], {from:payee}));
	});
});
