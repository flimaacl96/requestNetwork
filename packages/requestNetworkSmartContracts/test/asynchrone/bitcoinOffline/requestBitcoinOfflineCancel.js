...
contract('RequestBitcoinOffline Cancel',  function(accounts) {
	...
	it("cancel if Core Paused OK", async function () {
		...
		var r = await requestBitcoinOffline.cancelAction(utils.getRequestId(requestCore.address, 1), {from:payee});
		assert.equal(r.receipt.logs.length,1,"Wrong number of events");
		var l = utils.getEventFromReceipt(r.receipt.logs[0], requestCore.abi);
		assert.equal(l.name,"Canceled","Event Canceled is missing after cancelAction()");
	...
	it("cancel request Ethereum pause impossible", async function () {
		...
		await utils.expectThrow(requestBitcoinOffline.cancelAction(utils.getRequestId(requestCore.address, 1), {from:payee}));
	});

	it("cancel request not exist impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.cancelAction(666, {from:payer}));
	});

	it("cancel request from a random guy impossible", async function () {
		await utils.expectThrow(requestBitcoinOffline.cancelAction(utils.getRequestId(requestCore.address, 1), {from:burnerContract}));
	});
