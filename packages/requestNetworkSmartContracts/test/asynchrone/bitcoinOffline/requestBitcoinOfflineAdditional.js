...
contract('RequestBitcoinOffline AdditionalAction',  function(accounts) {
	...
	beforeEach(async () => {
		...
		await requestBitcoinOffline.createRequestAsPayeeAction(
								[payee,payee2,payee3],
	...
	it("can make additionals if Core Paused", async function () {
		...
		var r = await requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1),[arbitraryAmount10percent], {from:payer});
		...
		assert.equal(newReq[1],requestBitcoinOffline.address,"new request wrong data : currencyContract");
	...
	it("cannot make additionals if request does not exist", async function () {
		await utils.expectThrow(requestBitcoinOffline.additionalAction(666, [arbitraryAmount10percent], {from:payer}));
	});
	...
	it("can make additionals request with the currencyContract untrusted", async function () {
		await requestCore.adminRemoveTrustedCurrencyContract(requestBitcoinOffline.address, {from:admin});
		var r = await requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1),[arbitraryAmount10percent], {from:payer});
	...
	it("cannot make additionals on request canceled", async function () {
		await requestBitcoinOffline.cancelAction(utils.getRequestId(requestCore.address, 1), {from:payee});
		await utils.expectThrow(requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1), [arbitraryAmount10percent], {from:payer}));
	});

	it("cannot make additionals on request from a random guy", async function () {
		await utils.expectThrow(requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1), [arbitraryAmount10percent], {from:burnerContract}));
	});

	it("cannot make additionals on request from payee", async function () {
		await utils.expectThrow(requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1), [arbitraryAmount10percent], {from:payee}));
	});

	it("cannot make negative additionals on main payee", async function () {
		await utils.expectThrow(requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1), [-1], {from:payer}));
	});

	it("cannot make negative additionals on sub payee", async function () {
		await utils.expectThrow(requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1), [0, -1], {from:payer}));
	});

	it("can make additionals on accepted request", async function () {
		await requestBitcoinOffline.acceptAction(utils.getRequestId(requestCore.address, 1), {from:payer});
		var r = await requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1),[arbitraryAmount10percent], {from:payer});
	...
	it("can make additionals with amount > expectedAmount", async function () {
		var r = await requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1),[arbitraryAmount+1], {from:payer});
	...
	it("can make additionals with amount <= expectedAmount", async function () {
		var r = await requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1),[arbitraryAmount], {from:payer});
	...
	it("can make additionals on subPayees", async function () {
		var r = await requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1),[0,arbitraryAmount10percent,arbitraryAmount20percent], {from:payer});
	...
	it("cannot make additionals on more subPayees than expected", async function () {
		await utils.expectThrow(requestBitcoinOffline.additionalAction(utils.getRequestId(requestCore.address, 1),[0,arbitraryAmount10percent,arbitraryAmount20percent,arbitraryAmount30percent], {from:payer}));
	});
});
