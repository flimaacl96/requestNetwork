...
const addressRequestBitcoinOffline = requestArtifacts('private', 'last-RequestErc20-'+ADDRESS_TOKEN_TEST).networks.private.address;
...
describe('erc20 accept', () => {
    ...
    beforeEach(async () => {
        ...
        const req = await rn.requestBitcoinOfflineService.createRequestAsPayee(
            ADDRESS_TOKEN_TEST,
            [payee],
            [arbitraryAmount],
            payer,
    ...
    it('accept request with not valid requestId', async () => {
        try {
            const result = await rn.requestBitcoinOfflineService.accept(
                                '0x00000000000000',
                                {from: payer});
    ...
    it('accept request', async () => {
        ...
        expect(result.request.currencyContract.address.toLowerCase(), 'currencyContract is wrong').to.equal(addressRequestBitcoinOffline);
    ...
    it('accept request not in created state', async () => {
        try {
            // accept first
            await rn.requestBitcoinOfflineService.accept(
                                requestId,
                                {from: payer});
