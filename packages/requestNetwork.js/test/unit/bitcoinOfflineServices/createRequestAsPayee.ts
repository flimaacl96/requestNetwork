...
const addressRequestBitcoinOffline = requestArtifacts('private', 'last-RequestBitcoinOffline').networks.private.address;
...
describe('bitcoinOffline createRequestAsPayeeAction', () => {
    ...
    web3 = rn.requestBitcoinOfflineService.web3Single.web3;
    BitcoinServiceTest.init();
    rn.requestBitcoinOfflineService.bitcoinService = BitcoinServiceTest.getInstance();
    ...
    it('create request (implicit parameters)', async () => {
        const result = await rn.requestBitcoinOfflineService.createRequestAsPayee(
        ...
        expect(result.request.currencyContract.address.toLowerCase(), 'currencyContract is wrong').to.equal(addressRequestBitcoinOffline.toLowerCase());
